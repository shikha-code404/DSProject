...
export default function Login() {
  const { login, user, loginWithOAuth } = useAuth()
          ...
          <div className="w-full md:w-7/12 p-8 sm:p-12 lg:p-16 flex flex-col justify-center bg-white dark:bg-[#1E293B]">
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-5">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '12px' }}>
                <button
                  type="button"
                  onClick={async () => {
                    try { await loginWithOAuth('github') }
                    catch (err) { addToast({ type: 'error', title: 'GitHub login failed', message: err.message }) }
                  }}
                  className="btn-secondary w-full rounded-2xl justify-center gap-2 py-3 hover:bg-slate-50 dark:hover:bg-white/5 border-slate-200 dark:border-white/10"
                >
                  <Terminal className="w-5 h-5" /> Continue with GitHub
                </button>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    catch (err) { addToast({ type: 'error', title: 'Google login failed', message: err.message }) }
                  }}
                  className="btn-secondary w-full rounded-2xl justify-center gap-2 py-3 hover:bg-slate-50 dark:hover:bg-white/5 border-slate-200 dark:border-white/10"
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="btn-secondary rounded-2xl justify-center gap-2 py-3 hover:bg-slate-50 dark:hover:bg-white/5 border-slate-200 dark:border-white/10"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
